...
export const ResizePicture = (_targetType, _targetData, _modelService, commandService) => () => {
    ...
    <div className="w-full h-full flex flex-col bg-white">
      ...
      <div className="w-full flex flex-col"
        ...
        <div className="w-full mt-4 flex items-center text-sm text-black-light">
          <span className="mr-3">图片宽度</span>
          <InputNumber className="flex-1" value={targetData.params_user_resize_x} onChange={e => changeLabel(e,'params_user_resize_x')} size="middle"/>
        </div>
        {/*图片高度*/}
        <div className="w-full mt-4 flex items-center text-sm text-black-light">
          <span className="mr-3">图片高度</span>
          <InputNumber className="flex-1" value={targetData.params_user_resize_y} onChange={e => changeLabel(e,'params_user_resize_y')} size="middle"/>
        </div>
      </div>
    </div>
  )
